...
struct vec4 {
	...
	explicit vec4(float const* v) :vec4(v[0], v[1], v[2], v[3]) {}
...
public:
	...
	LineStrip(const LineStrip &ls) 
		:vertexData(ls.vertexData), created(false)
	{ 
		Create();
		glBindVertexArray(vao);
		glBindBuffer(GL_ARRAY_BUFFER, vbo);
	...
	void AddPoint(float cX, float cY) {
		...
		vertexData.push_back(wVertex.v[0]); // x
		vertexData.push_back(wVertex.v[1]); // y
		vertexData.push_back(1); // r
		vertexData.push_back(1); // g
		vertexData.push_back(0); // b
	...
	float length() const {
		if(vertexData.size() < 2 * ELEMENTS_PER_VERTEX) return 0.0f;
	}

	size_t points() const {
		return vertexData.size() / ELEMENTS_PER_VERTEX;
	}
	...
	friend std::ostream &operator<<(std::ostream &os, const LineStrip &ls) {
		os << "points: ";
		for(int i = 0; i < ls.vertexData.size() / 5; ++i) {
			os << '(' << ls.vertexData[i] << ',' << ls.vertexData[i+1] << ") ";
		}
		return os;
	}
};
...
class LagrangeCurve {
	...
	float L(int i, float t) const {
		float Li = 1.0f;
		for(int j = 0; j < controlPoints.size(); ++j) {
			if(j == i) continue;
			Li *= (t - params[j]) / (params[i] - params[j]);
		}
		return Li;
	}
public:
	void AddControlPoint(const vec4 &cp) {
		AddControlPoint(cp, controlPoints.size());
	}

	void AddControlPoint(const vec4 &cp, float param) {
	...
	vec4 r(float t) const {
		...
		for(int i = 0; i < controlPoints.size(); ++i) {
			result += controlPoints[i] * L(i, t);
		}
		return result;
	}

	LineStrip tesselate() const {
		LineStrip result;
		result.Create();
		for(float t = 0.0f; t <= 1.0f; t += 0.01f) {
			vec4 point = r(t / static_cast<float>(params.size()));
	...
	friend std::ostream &operator<<(std::ostream &, const LagrangeCurve &);
};

std::ostream &operator<<(std::ostream &os, const LagrangeCurve &curve) {
	os << "Control points: ";
	for(const auto &cp: curve.controlPoints) {
		os << '(' << cp[0] << ',' << cp[1] << ',' << cp[2] << ',' << cp[3] << ") ";
	}
	return os;
}
...
void onMouse(int button, int state, int pX, int pY) {
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {  // GLUT_LEFT_BUTTON / GLUT_RIGHT_BUTTON and GLUT_DOWN / GLUT_UP
		...
		curve.AddControlPoint(vec4(cX, cY));
		std::cout << curve << std::endl;
		lineStrip.AddPoint(cX, cY);
		lineStrip = curve.tesselate();
		lineStrip.Create();
